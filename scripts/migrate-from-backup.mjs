 ...
 *
 * DATABASE_PATH dibaca dari environment, atau dari file .env di root project
...
if (!process.env.DATABASE_PATH && existsSync('.env')) {
...
const stats = {}
/**
 * Bulk insert satu tabel — satu prepared statement dipakai ulang untuk semua baris,
 * jauh lebih cepat daripada prepare per-baris. Tidak digabung jadi satu INSERT
 * multi-VALUES supaya jumlah bound parameter tidak menabrak SQLITE_MAX_VARIABLE_NUMBER.
 */
...
// PRAGMA foreign_keys tidak bisa diubah di dalam transaksi — set sebelum BEGIN.
conn.pragma('foreign_keys = OFF')
conn.exec('BEGIN')

try {
  if (reset) {
    for (const t of TABLES_CHILD_FIRST) conn.exec(`DELETE FROM "${t}"`)
    console.log('  (--reset) seluruh tabel data dikosongkan, tabel users tidak disentuh')
    console.log('')
  }

  // ---------- bank_groups ----------
  insertMany('bank_groups', ['id', 'nama', 'warna'],
    (data.bankGroups || []).map(g => [g.id, g.nama, g.warna || null]))

  // ---------- bank_balances (dari array `bank[]` lama, panel "Saldo Rekening Bank") ----------
  insertMany('bank_balances', ['id', 'pic', 'rek', 'saldo', 'bisa_dipakai', 'ket', 'grup'],
  ...
  insertMany('bank_accounts', ['id', 'group_id', 'bank_type', 'nama_rek', 'no_rek', 'saldo_awal'],
    (data.bankAccounts || []).map(a => [a.id, a.grup || null, a.bankType || '', a.namaRek || '', a.noRek || '', a.saldoAwal ?? null]))

  // ---------- bank_txns ----------
  insertMany('bank_txns',
  ...
  insertMany('deposito_rows', ['id', 'nama', 'nominal', 'tgl_masuk', 'rate', 'jatuh_tempo', 'ket'],
    (data.deposito || []).map(d => [genId('dep'), d.nama || '', d.nominal || 0, d.tglMasuk || null, d.rate || '', d.jatuhTempo || null, d.ket || '']))
  insertMany('hutang_rows', ['id', 'peminjam', 'kreditur', 'nominal', 'rate', 'tgl_pinjam', 'jatuh_tempo', 'ket'],
    (data.hutang || []).map(h => [genId('hut'), h.peminjam || '', h.kreditur || '', h.nominal || 0, h.rate || '', h.tglPinjam || null, h.jatuhTempo || null, h.ket || '']))
  insertMany('bayar_rows', ['id', 'pt', 'nominal', 'tgl_bayar', 'tgl_pesan', 'no_ctr', 'pay_iam', 'pay_ekspds', 'ket'],
  ...
  insertMany('mp_stores', ['id', 'group_id', 'nama', 'platform', 'saldo_awal'],
  ...
  insertMany('mp_entries', ['id', 'store_id', 'tanggal', 'debet', 'kredit'], mpEntries)

  // ---------- npwp_master, coa_master, tag_master, aset_simple_master ----------
  insertMany('npwp_master', ['id', 'no_npwp', 'nama_npwp', 'nik', 'alamat'],
    (data.npwpMaster || []).map(n => [n.id, n.noNpwp || '', n.namaNpwp || '', n.nik || '', n.alamat || '']))
  insertMany('coa_master', ['id', 'no_coa', 'nama_coa'],
    (data.coaMaster || []).map(c => [c.id, c.noCoa || '', c.namaCoa || '']))
  insertMany('tag_master', ['id', 'nama'],
    (data.tagMaster || []).map(nama => [genId('tag'), nama]))

  insertMany('aset_simple_master', ['id', 'kind', 'value'], [
  ...
  insertMany('avp_rows', ['id', 'coa_id', 'group_id', 'tanggal', 'code', 'store', 'description', 'tags', 'debet', 'kredit'],
  ...
  insertMany('avp_lawan', ['id', 'row_id', 'partner_id'], lawan)

  // ---------- te_gudang_rows, te_finance_rows (Tagihan Ekspedisi) ----------
  insertMany('te_gudang_rows', ['id', 'tanggal', 'nama_pengirim', 'nama_penerima', 'inv_gii', 'no_waybill', 'biaya', 'keperluan'],
    (data.teGudangRows || []).map(r => [r.id, r.tanggal || null, r.namaPengirim || '', r.namaPenerima || '', r.invGii || '', r.noWaybill, r.biaya || 0, r.keperluan || '']))
  insertMany('te_finance_rows', ['id', 'tanggal', 'no_waybill', 'biaya', 'nama_penerima', 'keterangan'],
  ...
  insertMany('aset_depresiasi_log', ['id', 'aset_id', 'periode', 'tanggal'], depLog)

  // ---------- period_lock ----------
  if (data.periodLock) {
    conn.prepare('REPLACE INTO period_lock (id, lock_ym) VALUES (1, ?)').run(data.periodLock)
