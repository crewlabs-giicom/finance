...
import { csvParseLines, detectCsvFormat, deriveKetTransaksi, parseBcaCsv, parseBriCsv, txnDupKey } from '../../utils/bankCsv'

/**
 * Import mutasi bank dari CSV BCA atau BRI.
...
export default defineEventHandler(async (event) => {
  ...
  if (!format) {
    throw createError({ statusCode: 400, statusMessage: 'Format file tidak dikenali. Pakai file mutasi asli dari BCA atau BRI.' })
  }
  ...
  function push(acc: typeof accounts[number], t: { tanggal: string; transaksi: string; cabang: string; debet: number; kredit: number }, saldoAwal: number | null) {
    if (lockYm && t.tanggal.slice(0, 7) <= lockYm) { locked++; return }
    const key = txnDupKey({ accountId: acc.id, ...t })
    ...
    toInsert.push({
      ...
      bankType: acc.bankType, noBankManual: '',
